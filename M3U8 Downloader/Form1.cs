    ...
    public partial class Form1 : Form
    {
        ...
        private void button_Quit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (MessageBox.Show("The download process has started, are you sure you want to exit?\n（This may be mandatory）", "Are you sure?", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.Yes)
                    {
                        Stop();
                        MessageBox.Show("Command Sent! \n Please exit if this windows doesnt go away.", "Are you sure?");
                        try
                        {
        ...
        private void textBox_Info_TextChanged(object sender, EventArgs e)
        {
            ...
            label5.Text = "[Time elapsed：" + duration.Match(textBox_Info.Text).Value.Replace("Duration: ", "") + "]";
            ...
            { label6.Text = "[Size Downloaded：" + time.OfType<Match>().Last() + "，" + FormatFileSize(Convert.ToDouble(sizekb.OfType<Match>().Last().ToString().Replace("kB time", "")) * 1024) + "]"; }
            ...
            label7.Text = "[Download info：" + resolution.Match(textBox_Info.Text).Value.Replace(", ","") + "，" + fps.Match(textBox_Info.Text).Value.Replace(", ", "") + "]";
            if (time.Count > 0 && sizekb.Count > 0)  //防止程序太快 无法截取
            {
                ...
                + Convert.ToDouble(label5.Text.Substring(11, 2)) + Convert.ToDouble(label5.Text.Substring(14, 2)) / 100);
                    ...
                    this.Text = "Completed：" + String.Format("{0:F}", Progress) + "%" + " [" + FormatFileSize((big - small) * 1024) + "/s]";
                }
                catch(Exception)
                {
                    try
                    {
                        label5.Text = "[Total Durration：NULL]";
                        Double Downloaded = Convert.ToDouble(Convert.ToDouble(label6.Text.Substring(5, 2)) * 60 * 60 + Convert.ToDouble(label6.Text.Substring(8, 2)) * 60
                    + Convert.ToDouble(label6.Text.Substring(11, 2)) + Convert.ToDouble(label6.Text.Substring(14, 2)) / 100);
                        Double Progress = 100;
        ...
        private void button_Exit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Process.GetProcessById(ffmpegid) != null)
                {
                    if (MessageBox.Show("The download has started, do you still want to exit?\n", "Are you sure?", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.Yes)
                    {
                        Stop();
                        MessageBox.Show("Command Sent! \nPlease exit if this windows doesn't go away", "Are you sure?"); 
                        try
                        {
                            if (Process.GetProcessById(ffmpegid) != null)  //如果进程还存在就强制结束它
                            {
        ...
        {
            MessageBox.Show("Compiled on: 1-23-2020\nCopyright © 2016 nilaoda\nModifications and translations to the UI made by:\nSamuel Rathbun", "About", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        {
            String LogName = "Log-" + System.DateTime.Now.ToString("yyyy.MM.dd-HH.mm.ss") + ".txt";
            StreamWriter log = new StreamWriter(LogName);
            log.WriteLine("━━━━━━━━━━━━━━\r\n"
                + "■M3U8 Downloader User Log\r\n\r\n"
                + "■" + System.DateTime.Now.ToString("F") + "\r\n\r\n"
                + "■URL：" + textBox_Adress.Text + "\r\n\r\n"
                + "■Output：" + textBox_DownloadPath.Text + "\\" + textBox_Name.Text + houzhui.Text + "\r\n\r\n"
                + "■FFmpeg command: ffmpeg " + Command.Text + "\r\n"
                + "━━━━━━━━━━━━━━"
                + "\r\n\r\n"
                + textBox_Info.Text);
            log.Close();
            MessageBox.Show("The log has been created! ", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                if (Process.GetProcessById(ffmpegid) != null)
                {
                    ...
                    {
                        Stop();
                        MessageBox.Show("Command Sent! \nPlease exit if this window doesnt go away", "Are you sure?");
                        try
                        {
                            if (Process.GetProcessById(ffmpegid) != null)  //如果进程还存在就强制结束它
                            {
...
namespace M3U8_Downloader
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Exist_Run(string FileName)
        {
            ...
            {
                MessageBox.Show("Could not find:" + FileName, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void CmdProcess_Exited(object sender, EventArgs e)
        {
            ...
            label8.Text = "";
            MessageBox.Show("Done!", "M3U8 Downloader", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);  // 执行结束后触发
        }  
    }
}